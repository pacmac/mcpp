...
TOOLS: list[dict[str, Any]] = [
    # ── Task tools (top-level grouping, was "context") ──
    {
        "name": "plan_task_new",
    ...
    },
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                "show_archived": {
                    "type": "boolean",
                    "description": "Include archived tasks (default: false, only active shown)"
                },
                "show_all": {
                    "type": "boolean",
    ...
    },
    {
        ...
        "description": "Archive a task (must not be the active task)",
        "inputSchema": {
            "type": "object",
            "properties": {
                "name": {
                    "type": "string",
                    "description": "Task name to archive"
                }
            },
            "required": ["name"]
        }
    ...
    },
    {
        ...
        "description": "Add or view notes on a task",
        "inputSchema": {
            "type": "object",
            "properties": {
                "text": {
                    "type": "string",
                    "description": "Note text to add (omit to read)"
                },
                "name": {
                    "type": "string",
                    "description": "Task name (omit for active task)"
    ...
    # ── Step tools (individual items, was "task") ──
    {
    ...
    },
    {
        ...
        "description": "Add or view notes on a step",
        "inputSchema": {
            "type": "object",
            "properties": {
                "text": {
...
def _agentmod_path() -> Path:
    """Resolve path to the agentmod package directory."""
    mcpp_tools_dir = Path(__file__).resolve().parent.parent
    agentmod_path = mcpp_tools_dir / "agentmod"
    if not agentmod_path.is_dir():
        agentmod_path = Path(__file__).resolve().parent.parent.parent / "agentmod"
    return agentmod_path
    ...
    try:
        ...
        try:
            if command == "task":
                if action == "new":
                    ...
                    if action == "archive":
                        ...
                        if not name:
                            conn.close()
                            return {"success": False, "error": "task name required"}
                        plan_ctx.archive_task(conn, name)
                        tasks = plan_ctx.list_tasks(conn, status_filter="active")
                        conn.close()
                        ...
                        result = plan_ctx.get_task_status(conn)
                        conn.close()
                        return {"success": True, "result": result}
                        ...
                        if name:
                            ...
                            task_id = plan_ctx.resolve_active_task_id(conn)
                        result = plan_ctx.get_task_show(conn, task_id)
                        conn.close()
                        return {"success": True, "result": result}
                        ...
                        if text:
                            plan_ctx.add_context_note(conn, text, context_ref=name)
                            notes = plan_ctx.list_context_notes(conn, context_ref=name)
                            conn.close()
                ...
                try:
                    if action == "list":
                        result = plan_ctx.list_steps(conn)
                        return {"success": True, "result": result}
                        ...
                        plan_ctx.switch_step(conn, number)
                        result = plan_ctx.get_step_summary(conn, step_number=number)
                        return {"success": True, "result": result}
                        ...
                        if number is None:
                            return {"success": False, "error": "step number required"}
                        plan_ctx.complete_step(conn, number)
                        result = plan_ctx.get_step_summary(conn, step_number=number)
                        return {"success": True, "result": result}
                        ...
                        step_id, step_number = plan_ctx.create_step(
                            conn, task_ref, title, description_md=description_md
                        )
                        result = plan_ctx.get_step_summary(conn, step_number=step_number)
                        return {"success": True, "result": result}
                        ...
                        while i < len(cmd_args):
                            if cmd_args[i] == "--task" and i + 1 < len(cmd_args):
                                ...
                                i += 1
                        plan_ctx.delete_step(conn, number, task_ref=task_ref)
                        result = plan_ctx.list_steps(conn)
                        return {"success": True, "result": result}
                        ...
                        if text:
                            plan_ctx.add_step_note(conn, text, step_number=number)
                            notes = plan_ctx.list_step_notes(conn, step_number=number)
                            return {"success": True, "result": {"notes": notes}}
                        else:
                            notes = plan_ctx.list_step_notes(conn, step_number=number)
                            return {"success": True, "result": {"notes": notes}}
...
def _cmd_readme(workspace_dir: str, args: dict[str, Any]) -> dict[str, Any]:
    ...
    # Resolve agentmod package directory (same logic as _run_plan_cmd)
    mcpp_tools_dir = Path(__file__).resolve().parent.parent
    agentmod_path = mcpp_tools_dir / "agentmod"
    if not agentmod_path.is_dir():
        agentmod_path = Path(__file__).resolve().parent.parent.parent / "agentmod"

    readme_path = agentmod_path / "README.md"
